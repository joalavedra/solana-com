...
budować i wysyłać transakcje w celu wywoływania programów Solana, w tym:
...
Podpisz i wyślij [transakcję](mention:transaction) do sieci. Klucz para
[sender](mention:sender) jest wymagana w tablicy podpisujących, aby autoryzować
przeniesienie SOL z ich konta.
...
wyszukiwania transakcji w Solana Explorer.
...
Wygeneruj nową parę kluczy do użycia jako portfel.

### !mint

Wygeneruj nową parę kluczy do użycia jako adres konta Mint, które ma zostać
utworzone.
...
Oblicz liczbę lamportów wymaganą do zwolnienia z opłaty za przechowywanie (rent
exemption).
