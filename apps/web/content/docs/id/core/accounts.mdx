...
[executable](#executable)-nya. Ini disebut akun program. (Untuk menyederhanakan,
Anda dapat menganggap akun program sebagai program itu sendiri.)
...
Program yang di-deploy menggunakan loader-v3 tidak berisi kode program di bidang
`data` mereka. Sebagai gantinya, `data` mereka menunjuk ke **akun data program**
terpisah, yang berisi kode program. (Lihat diagram di bawah.)

![Akun program dengan data. Data menunjuk ke akun data program terpisah](/assets/docs/core/accounts/program-account-expanded.svg)
...
`executable` diatur ke `true`, menunjukkan bahwa akun tersebut adalah program.
...
pembuatan akun status program sering diabstraksi, tetapi penting untuk memahami
