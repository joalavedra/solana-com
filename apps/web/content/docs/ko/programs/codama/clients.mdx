...
TypeScript 및 Rust 클라이언트를 생성합니다. 생성된 클라이언트에는 명령어 호출 및
계정 가져오기 기능이 포함되어 있어 프로그램 계정이나 instruction data를 수동으로
직렬화하고 역직렬화할 필요가 없습니다.
...
따라하려면 `example`라는 새 디렉토리를 만들고 코드 스니펫에서 `idl.json` 파일과
