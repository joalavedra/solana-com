...
Alla oleva esimerkki näyttää, kuinka Solana SDK:ta käytetään keypair-avaimen
...
valinnaista syötettä (siemeniä). Alla oleva esimerkki näyttää, kuinka Solana
...
- [`owner`](mention:owner): Tilin omistavan ohjelman tunnus
...
<Callout type="info">
  Vaikka tätä saldoa kutsutaan nimellä rent (vuokra), se toimii enemmän
  talletuksen tavoin, sillä koko saldon voi saada takaisin, kun tili suljetaan.
  (Nimi "rent" tulee nyt vanhentuneesta [rent epoch](#rent-epoch) -kentästä.)
</Callout>
...
- Data account: Tämä kenttä yleensä tallentaa tiladataa, joka on tarkoitettu
  luettavaksi.
...
Aiemmin tämä kenttä seurasi, milloin tilin pitäisi maksaa rent. Tämä
...
tilimallissa. (Samankaltainen kuin käyttöjärjestelmissä, joissa tyypillisesti on
...
#### Program state account -tili

Ohjelmat käyttävät data-tilejä tilansa ylläpitämiseen. Tehdäkseen niin, niiden
täytyy ensin luoda uusi data-tili. Program state account -tilin luomisprosessi
on usein abstrahoitu, mutta on hyödyllistä ymmärtää taustalla oleva prosessi.
