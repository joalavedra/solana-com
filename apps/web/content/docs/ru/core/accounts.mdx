...
### Программно производный адрес

[Программно производный адрес](/docs/core/pda) (PDA) — это адрес, который
детерминированно создается с использованием ID программы и одного или нескольких
дополнительных входных данных (seed). Пример ниже демонстрирует, как
использовать Solana SDK для создания программно производного адреса.
...
- [`executable`](mention:executable): Указывает, содержит ли аккаунт исполняемый
  бинарный файл
- [`rent_epoch`](mention:rent_epoch): Устаревшее поле эпохи аренды
...
[арендой](/docs/references/terminology#rent), который позволяет хранить его
данные в блокчейне. Аренда пропорциональна размеру аккаунта.

<Callout type="info">
  Хотя этот баланс называется арендой, он больше похож на депозит, так как весь
  баланс может быть возвращен, когда аккаунт закрывается. (Название "аренда"
  происходит от устаревшего поля [эпоха аренды](#rent-epoch).)
</Callout>
...
этот механизм сбора аренды был впоследствии устаревшим.
...
Разделение кода программы и её состояния — это ключевая особенность модели
аккаунтов Solana. (Аналогично операционным системам, которые обычно имеют
...
аккаунтом. (Для простоты можно считать, что программный аккаунт — это сама
программа.)
...
состояния программы часто абстрагирован, но полезно понимать его основы.
...
1. Вызвать [System Program](/docs/core/programs#the-system-program) для создания
   аккаунта. (System Program затем передает право собственности новой
   программе.)
