...
程序定义了每个 [指令](/docs/core/instructions) 的业务逻辑。当您提交一个
...
以下示例展示了如何在两个账户之间转移 SOL。每个账户都有一个所有者程序，只有该程序可以扣除账户的 SOL 余额。
...
生成一个新的 keypair 作为 `sender`。

### !接收者

生成一个新的 keypair 作为 `receiver`。
...
函数计算必须为 mint account 的数据分配多少 lamports。
...
将两个指令添加到一个交易中。这确保了账户创建和初始化是原子操作。（要么两个指令都成功，要么都不成功。）

这种方法在构建复杂的 Solana 交易时很常见，因为它保证了所有指令一起执行。
...
- [钱包](mention:wallet) 账户签署，作为 [交易费用](/docs/core/fees)
  和账户创建的付款方
- [铸币](mention:mint) 账户签署，以授权其地址用于新账户
