...
钱包是由[System Program](/docs/core/programs#the-system-program)拥有的账户。钱包主要用于持有 SOL 并签署交易。当 SOL 首次发送到一个新地址时，会自动创建一个系统账户。

下面的示例生成一个[新 keypair](mention:keypair)，[请求 SOL](mention:airdrop)为新的公钥地址提供资金，并[检索新资助钱包的账户数据](mention:info)。
...
`lamports`
字段包含账户的 SOL 余额，以[lamports](/docs/references/terminology#lamport)为单位。
...
`rentEpoch`
字段是一个遗留字段，来自已弃用的租金机制。（此字段为向后兼容而保留。）
...
Token Program 是一个可执行程序账户。程序账户与所有
...
`executable` 字段被设置为 `true`，这表明该账户的 `data` 字段包含可执行代码。
...
Program 包含定义如何创建和更新 mint account 的指令。
...
函数，用于帮助将 Mint 账户的 `data` 字段反序列化为由 Token Program 定义的
...
`supply`
字段显示了已铸造的代币总数。此值以代币的最小单位表示。要获得标准单位的总供应量，请根据
`decimals` 字段调整该值。
